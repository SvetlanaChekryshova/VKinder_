...
def create_buttons(number: int) -> VkKeyboard:
    ...
    if number == 1:
        ...
        keyboard.add_button('На основе моего профиля', buttons_colors[1])
        keyboard.add_line()
...
def add_data_to_the_dictionary(object_vk_api: object, index: int, sender_id: str,
                               message_text: str, date: dict, user_info: tuple) -> tuple:
    '''Добавляет ответы полученные от пользователя в словарь.'''
    print('message_text:', message_text)
    print('date:', date)
    print('user_info:', user_info)
    if index - 1 == 0:
        if message_text == 'на основе моего профиля':
            message_text = user_info[2]
        if '-' in message_text:
        ...
        if message_text == 'на основе моего профиля':
            message_text = '1' if user_info[4] == 'Мужской' else '2'
...
def event_handling_start(
        object_vk_api: object, message_text: str, variables: dict, user_info: tuple) -> dict:
    ...
    variables['filtr_dict'], variables['count'] = add_data_to_the_dictionary(
        ...
        sender_id, message_text, variables['filtr_dict'], user_info
    )
